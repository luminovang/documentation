...
1. **Avoid Repetition**
   Write code once and reuse it anywhere in your project. No need to copy the same logic in multiple files.

2. **Speed Up Development**
   Helpers take care of common tasks like formatting, file paths, or date checks, so you can focus on what matters.

3. **Improve Readability**
   Replacing long or complex logic with a short, clear function makes your code easier to understand and maintain.

---

## Custom Helper Functions and Constants

Custom global helper functions are **turned off by default** for safety.
To enable them, set this in your `.env` file:
...
1. **Check if the function or constant already exists** before defining it.
2. **Use a namespace** (e.g., `App\Utils\Globals`) to group your custom helpers.
3. **Use a prefix** in names to reduce the chance of naming collisions.
...
> **Recommendation**
>
>Use **Option 1** if your helpers are small and need to be truly global.
>Use **Option 2 or 3** for better organization and future-proofing, especially in larger projects.
...
These functions are available globally and **do not use any namespace**.
Calling them with a namespace (e.g., `Luminova\Funcs\function_name()`) will result in a **"function not found"** error.
...
[Environment Variables](/global/env-variables.md) - Learn more from documentation about Luminova's `env` variable keys.
...
Set an environment variable and persist it in the `.env` file:
...
The utility functions in the `Luminova\Funcs` namespace must be explicitly imported before use.
...
This function returns either a shared (singleton) or a new instance of the core application class. By default, Luminova's `CoreApplication` doesn't accept constructor arguments, but this function allows passing optional arguments to override or customize the instance.
...
`App\Application<Luminova\Foundation\Core\Application>` - Returns the shared instance if `$shared` is true, or a new instance if `$shared` is false.
...
[Core Application](/foundation/application.md) - See the documentation for base application methods and usages.
...
`Luminova\Http\Request<Luminova\Interface\HttpRequestInterface,Luminova\Interface\LazyObjectInterface>` - Returns instance of HTTP request class.

**See Also**

[HTTP Outgoing Request](/http/request.md) - See the documentation for http request methods and usages.
...
- [Luminova\Exceptions\AppException](/error-handlers/exceptions.md#appexception) - Throws an exception if factory context does not exist or error occurs.
...
-  `language`  - Returns instance of `Luminova\Component\Languages\Translator`
-  `logger`  - Returns instance of ` Luminova\Logger\Logger`
-  `fileManager`  - Returns instance of `Luminova\Utility\Storage\FileManager`
...
-  `escaper`  -  Returns instance of `Luminova\Common\Escape`
...
[Application Factory](/boot/factory.md) - See the documentation for factory methods and usages.
...
- [Luminova\Exceptions\AppException](/error-handlers/exceptions.md#appexception) - Throws an exception if service does not exist or error occurs.
...
- [Luminova\Exceptions\RuntimeException](/error-handlers/exceptions.md#runtimeexception) -Throws if layout file is not found.
...
`Luminova\Template\Response<Luminova\Interface\ViewResponseInterface>` - Return instance of view response object.
...
For more information see [View Response Class](/templates/response.md) documentation for and usage examples.
...
```php
function abort(int $status = 500, string|array|object|null $message = null, array $headers = [], ?string $type = null): never 
```
...
- [Luminova\Exceptions\RuntimeException](/error-handlers/exceptions.md#runtimeexception) - If error occur while sending abort response.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `$template` | **string** | The template filename to render. |
| `$status` | **int** | HTTP response status code (default: `200`). |
| `$options` | **array<string,mixed>** | Optional template scope data. |
| `$status` | **int** | Template rendering content type (default: `View::HTML`). |

**Return Value:**

`int` - Returns response status code `STATUS_SUCCESS` or `STATUS_SILENCE` if failed.
...
[User Agent Object](/http/user-agent.md) - See the documentation for browser user-agent methods and usages.
...
`string` - Return the generated reference to view, If `$absolute` is true, the absolute URL is returned, otherwise, a relative URL is returned.
...
This function generates a `URL` to a file within the assets folder, ensuring the base starts from your application's public document root. If `$filename` is null, it links to the base assets directory.
...
```
> **Note:**
> If `$absolute` is true, the absolute URL is returned, otherwise a relative URL is returned.
...
It attempts to locate the root by checking for a `.env` file in `APP_ROOT`, the parent directory, or by walking up the directory tree.
...
```php
$logPath = Luminova\Funcs\root('../example.com/writeable/logs', 'debug.log');

// Returns: /path/to/www/../example.com/writeable/logs/debug.log
```

> **Note:** 
> If you pass a filename as `$path` instead of using `$filename`, it will treat it as a folder
...
[File System Manager](/files/file-manager.md) - Also checkout the documentation for file manager methods and usages.
...
| `$scope` | **array<string,mixed>** | Optional associative array of variables to extract into the file scope.<br/>(Keys become variable names). |
...
- [Luminova\Exceptions\RuntimeException](/error-handlers/exceptions.md#runtimeexception) - If file doesn't exist and `$throw` is true.
- [Luminova\Exceptions\InvalidArgumentException](/error-handlers/exceptions.md#invalidargumentexception) - If `$scope` is not empty and not an associative array (list array given).
...
- [Luminova\Exceptions\RuntimeException](/error-handlers/exceptions.md#runtimeexception) - If the file is missing and $throw is true.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `$to` | **string** | The destination (e.g, Log level, email, or endpoint to send the log). |
...
- [Luminova\Exceptions\InvalidArgumentException](/error-handlers/exceptions.md#invalidargumentexception) -If the target is invalid or logging fails.

> **Note:**
> It uses the configured preferred PSR logger class if define in `App\Config\Preference`, otherwise it will use default `Luminova\Logger\NovaLogger`.
>
> All non-external log destination are located in `/writeable/log/`.
...
- [Luminova\Exceptions\InvalidArgumentException](/error-handlers/exceptions.md#invalidargumentexception) - f the namespace or name is not provided for versions 3 or 5.

> **Note:**
> To check if `UUID` is valid use [Common Helpers](/common-functions.md)
...
- [Luminova\Exceptions\InvalidArgumentException](/error-handlers/exceptions.md#invalidargumentexception) -  When an invalid, blank encoding is provided or unsupported encoding or empty string is provided.
- [Luminova\Exceptions\BadMethodCallException](/error-handlers/exceptions.md#badmethodcallexception) - When an invalid context is called.
...
- [Luminova\Exceptions\InvalidArgumentException](/error-handlers/exceptions.md#invalidargumentexception) - Throws if the input does not match the expected type and no replacement is provided.
...
- [Luminova\Exceptions\NotFoundException](/error-handlers/exceptions.md#notfoundexception) - If translation is not found and no default is provided.
...
- [Luminova\Exceptions\ClassException](/error-handlers/exceptions.md#classexception) - If unsupported driver is specified.
- [Luminova\Exceptions\CacheException](/error-handlers/exceptions.md#cacheexception) - If there is an issue initializing the cache.
- [Luminova\Exceptions\InvalidArgumentException](/error-handlers/exceptions.md#invalidargumentexception) - If an invalid subdirectory is provided for the filesystem cache.
...
- [Luminova\Exceptions\FileException](/error-handlers/exceptions.md#fileexception) - If unable to write file.

**See Also**

[File Manager](/files/file-manager.md) - Also checkout the documentation for file manager methods and usages.
...
- [Luminova\Exceptions\FileException](/error-handlers/exceptions.md#fileexception) - If unable to create directory.
- [Luminova\Exceptions\RuntimeException](/error-handlers/exceptions.md#runtimeexception) - If the path is not readable.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `$input` | **array&#124;string** | Input array or listify string from `Luminova\Utility\Collections\Listifier` (e.g, `foo=bar,bar=2,baz=[1;2;3]`). |
...
The function uses `Luminova\Utility\Collections\Listifier` to convert a string list format into an array.
...
> In Luminova, a string `list` or `Listifier` typically consist of comma-separated values, which may or may not be enclosed in quotes (e.g, `Foo, Bar, Baz`, `"Foo", "bar", "Baz"`).
> For more information refer to [String List](/utilities/string-listification.md) documentation 
