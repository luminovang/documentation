...
- **`database.emulate.prepares`** **(bool)**:
  - **Description**: Enables or disables the emulation of prepared statements in the database driver.
    When set to `true`, it allows the use of the same named parameter multiple times in a query (e.g., `:id` in multiple places). This is supported also in MySQLi driver that do not support repeated named parameters natively.
    Set to `false` to use native prepare behavior, where each placeholder must be unique.
...
- **`database.pdo.version`** **(string)**:
  - **Description**: Specifies the `PDO`  DBMS version or driver type to use. Available database (`mysql`, `sqlite`, `sqlsrv`, `pgsql`, `cubrid`, `dblib`, or `oci`).
