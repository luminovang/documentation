...
The **JWT** class is designed to handle the creation, decoding, and validation of JSON Web Tokens (JWT) for user authentication and authorization within the Luminova framework. It provides an easy way to manage JWTs using various signing algorithms and ensure secure token-based authentication for your applications.
...
The `JWT` class can be instantiated either using the constructor or through the `getInstance()` method, which provides a singleton instance of the class.
...
* Class namespace: `Luminova\Security\JWT`
...
Initialize the JWT class constructor with configurable JWT and key settings.
...
- [Luminova\Exceptions\EncryptionException](/error-handlers/exceptions.md#encryptionexception) - If the provided $path is not readable or writable.

***

### getInstance

Get or create a singleton instance of the JWT class.

This method implements the Singleton pattern, ensuring that only one instance
of the JWT class exists throughout the application's lifecycle.
...
`self` -  Returns the singleton instance of the JWT class.
...
- [Luminova\Exceptions\EncryptionException](/error-handlers/exceptions.md#encryptionexception) - Throws on development mode if an error is encountered while decoding.
...
- [Luminova\Exceptions\EncryptionException](/error-handlers/exceptions.md#encryptionexception) - If the `$user_id` is empty or invalid.
...
```php
$filename = JWT::filename('user-id');
// Example output: 'c4ca4238a0b923820dcc509a6f75849b.key'
```
With plain filename:

```php
$filename = JWT::filename('user-id', false);
...
- [Luminova\Exceptions\EncryptionException](/error-handlers/exceptions.md#encryptionexception) - If the user identifier is empty or the application key is missing.
...
- [Luminova\Exceptions\EncryptionException](/error-handlers/exceptions.md#encryptionexception) Throws on development mode if an error occurs while saving the key.
