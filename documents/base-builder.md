...
The Luminova **Document Builder** class is a foundational helper for both `Luminova\Component\Widget\XHTML` and `Luminova\Component\Widget\Inputs`, which extend it to inherit dynamic configuration capabilities. The Document class includes core utilities, such as a `calendar` method for generating HTML calendar UIs, along with other helper functions.
...
* Class namespace: `Luminova\Component\Widget\Document`
...
The document element style to use in `Luminova\Component\Widget\Xhtml` class (e.g, `Xhtml::HTML5`, `Xhtml::BOOTSTRAP5`).
...
**Throws:**
[Luminova\Exceptions\DateTimeException](/error-handlers/exceptions.md#datetimeexception) Throws if invalid calendar date format is provided.

**Example:**
```php
<?php
use Luminova\Component\Widget\Document;
