...
It includes various methods to specify execution intervals (e.g., seconds, minutes, days), logging, and handling completion or failure events. For more details see [Abstract Cron Task Queue](/foundation/cron-queue.md) documentation.

---

### Defining a Cron Service

To schedule specific services for execution in a cron job, use the `schedule` method provided in the [App\Config\Cron](/configs/cron-tasks.md) class. This method allows you to define the command controller class and the corresponding method to execute using the `service` method of the [Cron Tasks Command Line Execution](/task-queue-system/cron-task-execution.md).
...
Here is a full example that demonstrates all available methods, to learn more about the method see [Core Cron Tasks Documentation](/foundation/cron-queue.md).
