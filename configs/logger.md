...
The `Logger` configuration class allows you to set a preferred PSR-compliant logger for your application, replacing the default [Nova Logger](/logging/nova-logger.md) system.

It also supports asynchronous logging via PHP `Fiber`, allowing log messages to be processed in the background without blocking execution, improving performance for high-traffic or long-running tasks.
