...
The `ContentSecurityPolicy` class extends the `Luminova\Base\Configuration` class to provide a configuration of Content-Security-Policy (CSP) customized for web applications. It allows you to sets up default directives for security and easily modify or add more directives as needed.
...
2. **Add/Remove Directives**:
   You can add or remove specific directives using the methods provided by `Configuration`.
...
You can extend `ContentSecurityPolicy` or `Configuration` to further customize the CSP for specific use cases. For instance:
