...
Cookie file jar provides an easy way to manage cookies when making HTTP requests with Luminova **Novio** HTTP client or retrieving incoming HTTP request cookie header.

The Luminova framework simplifies cookie management during HTTP requests using the `Luminova\Http\Client\Novio` client. The **Cookie File Jar** class provides a interface and methods for handling cookies by storing them in a file and reusing them across multiple requests. This makes it ideal for maintaining session states or persisting authentication cookies.

Additionally, the `FileJar` class processes incoming HTTP requests that include a `Cookie` header. It integrates with the [Novio HTTP Request Client](/http-client/novio.md), enabling easy access to all cookie information associated with the current request object.
...
Below is an example that demonstrates how to use the `FileJar` with Luminova Novio HTTP client class:
...
// Create a File Jar to manage cookies
$cookies = new FileJar(root('/writeable/temp/', 'cookies.txt'), [
...
$network = new Network(new Novio([
    'headers' => [
        'Content-Type' => 'application/json'
    ],
    'cookies' => $cookies, // Pass the FileJar instance
]));
...
* Class namespace: `Luminova\Cookies\FileJar`
* This class implements:
[Luminova\Interface\CookieJarInterface](/interface/classes.md#cookiejarinterface), [Luminova\Interface\LazyObjectInterface](/interface/classes.md#lazyobjectinterface), [\Stringable](https://www.php.net/manual/en/class.stringable.php), [\Countable](https://www.php.net/manual/en/class.countable.php)
...
```php
$cookieJar = new FileJar('path/to/cookie.txt', [
    'name' => 'SessionID',
    'domain' => '.example.com',
    'path' => '/',
    'newSession' => false,
...
- [Luminova\Exceptions\CookieException](/error-handlers/exceptions.md#cookieexception) - Throws if called without cookie name specified.
...
- [Luminova\Exceptions\CookieException](/error-handlers/exceptions.md#cookieexception) - If the domain is empty.
...
| `$metadata` | **bool** | Whether to include metadata (e.g., `Expires`, `Max-Age` etc...).<br/>This can be used for testing purposes or to emulate a browser's behavior.. |
...
- [Luminova\Exceptions\CookieException](/error-handlers/exceptions.md#cookieexception) - If no valid domain is found or set.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `$as_array` | **bool** | Whether to return value as an array if its a valid json string (default: false). |
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `$return_string` | **bool** | Whether to retrieve unix-timestamp or formate cookie datetime string |
