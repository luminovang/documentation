...
The **Http Client Server** class is designed to provide a flexible and efficient HTTP server for handling incoming requests. This class allows you to create a server that can manage different connection types, handle both HTTP and WebSocket protocols, and apply various configurations such as rate limiting, timeouts, port management, sequential and simultaneous client connections. It is ideal for developers building lightweight web applications, services, or custom servers that need to manage client-server communication efficiently.
...
```php
use Luminova\Http\Client\Server;
...
* Class namespace: `Luminova\Http\Client\Server`
...
- [Luminova\Exceptions\RuntimeException](/error-handlers/exceptions.md#runtimeexception) - if the server fails to create a valid socket.
...
`self` - Returns instance of Http Server.
...
- [Luminova\Exceptions\RuntimeException](/error-handlers/exceptions.md#runtimeexception) - Throw if `$max` is not a positive integer.
...
- [Luminova\Exceptions\RuntimeException](/error-handlers/exceptions.md#runtimeexception) - If the server cannot bind to the address or port, or if the port is already in use.
