...
- **Type Safety**: Ensures compatibility with object type casting through the use of marker interface `LazyObjectInterface`.
...
* Class namespace: `Luminova\Utility\Object\LazyObject`
* This class implements:
[Luminova\Interface\LazyObjectInterface](/interface/classes.md#lazyobjectinterface), [\Stringable](https://www.php.net/manual/en/class.stringable.php)
...
- [Luminova\Exceptions\RuntimeException](/error-handlers/exceptions.md#runtimeexception) - If the class does not exist or an error occurs during initialization.
...
- [Luminova\Exceptions\RuntimeException](/error-handlers/exceptions.md#runtimeexception) - If the lazy ghost creation fails.
...
- [Luminova\Exceptions\RuntimeException](/error-handlers/exceptions.md#runtimeexception) - If error occurs while initializing class.
...
- [Luminova\Exceptions\RuntimeException](/error-handlers/exceptions.md#runtimeexception) - If the class does not exist or error occurs.
...
The `LazyObjectInterface` in **Luminova** is a marker interface with no abstract methods. It allows your classes to support lazy initialization while maintaining compatibility with object type casting. 
...
You'll encounter an **error**. The property `$example` expects an instance of the `Example` class, but `LazyObject::newObject` returns an object or a class object that implements `LazyObjectInterface`. This type mismatch causes the issue.
...
#### 2. Implement the LazyObjectInterface

Update the `Example` class to implement the `LazyObjectInterface`:
...
class Example implements LazyObjectInterface {}
```
...
```php
protected ?LazyObjectInterface $example = null;
```

This allows `$example` to accept both `Example` and objects implementing `LazyObjectInterface`.
...
{
    /**
     * @var Example<LazyObjectInterface>|null $example
     */
    protected ?LazyObjectInterface $example = null;
...
Type Compatibility, to ensure that lazy-loaded instances work seamlessly with type hints. By implementing `LazyObjectInterface` and using the correct type casting, you can harness the benefits of lazy loading without compromising type safety.