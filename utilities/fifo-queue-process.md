...
* Class namespace: `Luminova\Utility\Queue`
...
- **$queue**: `Luminova\Utility\Queue` - The current instance of the queue class, allowing access to its properties and methods.
...
```php
<?php
$queue->onResponse(function (mixed $result, mixed $previous, Luminova\Utility\Queue $queue) {
...
```php
public getInstance(int $index): Luminova\Utility\Queue
```
...
`Luminova\Utility\Queue` - Return new Queue instance.
...
```php
public current(): Luminova\Utility\Queue
```
...
```php
public next(): Luminova\Utility\Queue
```
...
```php
public last(): Luminova\Utility\Queue
```
