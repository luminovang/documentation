...
Additionally, you can use magic getter methods like `HttpErrorCode::status200()` to directly retrieve the message of a specific status code.
...
Your method call must follow this pattern: `status followed by the http status code`. Example, `xxx` represent the status code you want to return it message the prefix `status` must come before the HTTP code `HttpErrorCode::status404()`.
...
```php
<?php
var_dump(HttpErrorCode::get());
// OR
var_dump(HttpErrorCode::$codes);
```
...
```php 
<?php 
echo HttpErrorCode::status200(); // OK
```
...
```php 
<?php 
echo HttpErrorCode::status503(); // Service Unavailable
```

```php
<?php
echo HttpErrorCode::get(400); // Bad Request
```