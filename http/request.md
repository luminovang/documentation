...
```php
use function Luminova\Funcs\request;
...
If your controller extends `Luminova\Base\Controller`.
...
* This class implements:  [Luminova\Interface\HttpRequestInterface](/interface/classes.md#httprequestinterface), [Luminova\Interface\LazyObjectInterface](/interface/classes.md#lazyobjectinterface), [\Stringable](https://www.php.net/manual/en/class.stringable.php)
...
| `$files` | **array<string,array>** | Optional request files, provided as an associative array (e.g., `['field' => [...]]`). |
...
// Or using ANY
echo $request->input('name') //John Doe
```
...
Get a field value from **Any** valid incoming HTTP request method.

If `$method` is provided, it checks field in the specified HTTP method, otherwise it use the current request method.

```php
public input(string $field, mixed $default = null, ?string $method = null): mixed
```
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `$field` | **string** | The input field name to retrieve the value value from. |
| `$default` | **mixed** | An optional default value to return if the field is not found (default: `null`). |
| `$method` | **string\|null** | Optional HTTP method to retrieve valid from (default: `null` as `ANY`). |

**Return Value:**

`mixed` - Return the value from HTTP request method body based on field name or default value.
...
```php
public getGet(string $field, mixed $default = null): mixed
```
...
```php
public getPost(string $field, mixed $default = null): mixed
```
...
```php
public getPut(string $field, mixed $default = null): mixed
```
...
```php
public getOptions(string $field, mixed $default = null): mixed
```
...
```php
public getPatch(string $field, mixed $default = null): mixed
```
...
```php
public getHead(string $field, mixed $default = null): mixed
```
...
```php
public getConnect(string $field, mixed $default = null): mixed
```
...
```php
public getTrace(string $field, mixed $default = null): mixed
```
...
```php
public getPropfind(string $field, mixed $default = null): mixed
```
...
```php
public getMkcol(string $field, mixed $default = null): mixed
```
...
```php
public getCopy(string $field, mixed $default = null): mixed
```
...
```php
public getMove(string $field, mixed $default = null): mixed
```
...
```php
public getLock(string $field, mixed $default = null): mixed
```
...
```php
public getUnlock(string $field, mixed $default = null): mixed
```
...
- [Luminova\Exceptions\InvalidArgumentException](/error-handlers/exceptions.md#invalidargumentexception) - Throw if field name is not provided.
...
```php
public getBody(bool $object = false, ?string $method = null): array|object
```
...
| `$method` | **string\|null** | Optional HTTP request method, if `nul`l current request method will be used (default: `null`). |
...
Retrieve the [CSRF token](/security/csrf.md) from the current request.

This method checks incoming request data **(POST, PUT, DELETE, or GET)** for a field named `csrf_token` and returns its value if present.
...
`string|null` - Returns the CSRF token string if found, otherwise `null`.

***

### getFields

Retrieve an array of request fields (`keys`).
...
`array` - Return an array list request fields.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `$extension` | **bool** | Whether to throw an exception if invalid host or not allowed host (default: false). |
...
- [Luminova\Exceptions\SecurityException](/error-handlers/exceptions.md#securityexception) - Throw if host is invalid or not allowed.
...
|-----------|------|-------------|
| `$extension` | **bool** | Whether to throw an exception if invalid host or not allowed host (default: false). |
| `$port` | **bool** | Whether to return hostname with port (default: true). |
...
- [Luminova\Exceptions\SecurityException](/error-handlers/exceptions.md#securityexception) - If host is invalid or not allowed.
...
- [Luminova\Exceptions\InvalidArgumentException](/error-handlers/exceptions.md#invalidargumentexception) - If invalid context is provided.
