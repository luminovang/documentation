...
| `$ifNotSent` | **bool** | Whether to send headers if headers is not already sent (default: true). |
| `$charset` | **bool** | Whether to append default charset from env to `Content-Type` if it doesn't contain it (default: false). |
