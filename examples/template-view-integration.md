...
Extending [Luminova\Base\Controller](/controllers/http-controller.md) automatically initializes the HTTP request class `Luminova\Http\Request` and the input validation class `Luminova\Security\Validation`.
...
Extending [Luminova\Base\Command](/controllers/cli-command-controller.md) allows the controller to handle command line operations in a manner similar to HTTP controllers. For more details on command line implementation, [see examples](/examples/novakit-command-usages.md).
...
Caching can be implemented either automatically or manually to learn more about view caching [read documentation](/cache/static-html-cache.md).
