...
2. **Create the Server**: Instantiate the `Http Server` class by specifying the `IP` address and `port`. For example, we’re binding it to `127.0.0.1` on port `8081`.

```php
use Luminova\Http\Client\Server;

$server = new Server('127.0.0.1', 8081, false);
```
