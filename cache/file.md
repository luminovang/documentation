...
* Parent class: `Luminova\Base\Cache`
...
These constants define default cache expiration times in seconds. They can be used throughout the application wherever cache TTL (time-to-live) is needed.

| Constant    | Type | Value            | Description                      |
| :---------- | :--- | :--------------- | :------------------------------- |
| `TTL_7DAYS` | int  | 7 * 24 * 60 * 60 | Represents 7 days in seconds     |
| `TTL_24HR`  | int  | 24 * 60 * 60     | Represents 24 hours in seconds   |
| `TTL_30MIN` | int  | 30 * 60          | Represents 30 minutes in seconds |
...
```php
use function Luminova\Funcs\cache;

$cache = cache('filesystem');
```

For more information on implementing cache features in your application, refer to the [Cache Examples Documentation](/examples/cache-integration.md).
...
```php
public __construct(?string $storage = null, ?string $subfolder = null): mixed
```
...
- [Luminova\Exceptions\CacheException](/error-handlers/exceptions.md#cacheexception) - if there is a problem loading the cache.
...
```php
public static getInstance(?string $storage = null, ?string $subfolder = null): static
```
...
- [Luminova\Exceptions\InvalidArgumentException](/error-handlers/exceptions.md#invalidargumentexception) - Throws if invalid sub directory is provided.
...
- [Luminova\Exceptions\CacheException](/error-handlers/exceptions.md#cacheexception) - Throws if cannot load cache or unable to process items.
- [Luminova\Exceptions\InvalidArgumentException](/error-handlers/exceptions.md#invalidargumentexception) - Throws if empty cache storage name is provided.
...
- [Luminova\Exceptions\CacheException](/error-handlers/exceptions.md#cacheexception) - Throws if called without any storage name specified or unable to store cache.
- [Luminova\Exceptions\InvalidArgumentException](/error-handlers/exceptions.md#invalidargumentexception) - Throws if empty cache key is provided.
...
```php
public execute(array<int,string> $keys, bool $withCas = false, ?callable $callback = null): bool
```
...
- [Luminova\Exceptions\InvalidArgumentException](/error-handlers/exceptions.md#invalidargumentexception) - Throws if keys is empty array.
...
- [Luminova\Exceptions\CacheException](/error-handlers/exceptions.md#cacheexception) - Throws if called without any storage name specified or error occurred while reading cache.
- [Luminova\Exceptions\InvalidArgumentException](/error-handlers/exceptions.md#invalidargumentexception) - Throws if empty cache key is provided.
...
- [Luminova\Exceptions\CacheException](/error-handlers/exceptions.md#cacheexception) - Throws if error occurred while deleting items.

> **Note:** 
> This method will remove item even if its was locked to prevent deletion.