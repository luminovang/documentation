...
Luminova supports version-controlled caching, allowing you to serve older cached content by listing previous app versions under the `page.cache.app.versions` setting.

For example, if your current app version is `1.2.0` but you have cached content from `1.0.0` and `1.1.0`, you can define:

```ini
page.cache.app.versions = [1.0.0, 1.1.0]
```

This tells framework cache system to look for cached content in those older versions when the current version has none.

> **Note:**
> Versioned caching is powerful, but to take full advantage of it, use versioned URLs.
> If not, non-versioned paths (like `/about`) will always return the latest version’s content because the key is the same.

**Example:**

* `https://example.com/1.0.0/about` and `https://example.com/1.1.0/about` — allows serving correct versioned content.
* ~~https://example.com/about~~ — Will always serves the latest version since the cache key doesn’t change.
