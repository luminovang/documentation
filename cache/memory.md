...
The Luminova **Memory Cache** class extends PHP's `Memcached` to simplify caching by storing and retrieving cached content on the system's hard disk. This class allows you to cache reusable data locally or remotely, reducing the need for repetitive database queries or API calls. By caching responses, you can improve performance and efficiency when the same content is requested multiple times.

This class shares core methods with the **File Cache** class, making it easy to switch between memory and file system caching without altering existing code. However, the **MemoryCache** class includes some unique methods not available in **Luminova\Cache\FileCache**, and vice versa.
...
## Setups
To setup the default memcached connection server, these are the [environment variable keys](/global/env-variables.md) required:
...
- [Luminova\Exceptions\CacheException](/error-handlers/exceptions.md#cacheexception) - if there is an issue loading the cache.
...
```php
public static getInstance(?string $storage = null, ?string $persistent_id = null): static
```
