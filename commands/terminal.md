...
When using the `Terminal` class **outside** of command controllers (such as `Luminova\Base\Command` or `Luminova\Base\Console`), you must first **initialize** the class manually.
...
In a **routable command controller** (a class extending `Luminova\Base\Command`), the `Terminal` methods can be accessed directly using `$this`, `self`, or `parent` keywords, as the base class exposes terminal functionality directly.
...
In a **console controller** (a class extending `Luminova\Base\Console`), use the `$this->term` property to access the `Terminal` instance.
...
| `$clear` | **bool** | Whether to clear the console screen and input while freezing (default: false). |
...
| `$multiChoice` | **bool** | Whether multiple options are allowed (default: true). |
...
- [Luminova\Exceptions\InvalidArgumentException](/error-handlers/exceptions.md#invalidargumentexception) - Throw if options is not specified or an empty array.
...
| `$maxLevel` | **int** | The maximum verbosity level (default: `3`). |
| `$default` | **int** | Default verbose level (default: `0`). |

**Return Value:**

`int` - Returns the verbosity level between 0 and `$max` or default if not specified.
